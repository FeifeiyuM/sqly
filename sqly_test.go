...
var opt = &Option{
	Dsn:             "root:mysql123@tcp(localhost:3306)/secretspace?charset=utf8mb4&collation=utf8mb4_unicode_ci&parseTime=true&loc=Local",
	DriverName:      "mysql",
	MaxIdleConns:    0,
	MaxOpenConns:    0,
	ConnMaxLifeTime: 0,
...
func TestSqlY_QueryOne(t *testing.T) {
	...
	acc := new(Account)
	query := "SELECT `id`, `nickname`, `avatar`, `email`, `mobile`, `password`, `role` FROM `accounts` " +
		"WHERE `id`=?;"
...
func TestSqlY_Update(t *testing.T) {
	...
	query := "UPDATE `accounts` SET `nickname`=? WHERE `mobile`=?;"
...
func TestSqlY_Delete(t *testing.T) {
	...
	query := "DELETE FROM `accounts` WHERE `mobile`=?;"
...
func TestSqlY_ExecManyCtx(t *testing.T) {
	...
	ctx := context.TODO()
	var queries []string
	query, _ := QueryFmt("UPDATE `accounts` SET `nickname`=? WHERE `mobile`=?;", "nick_many", "18112342345")
	queries = append(queries, query)
	query, _ = QueryFmt("DELETE FROM `accounts` WHERE `mobile`=?;", "18112342346")
	queries = append(queries, query)
